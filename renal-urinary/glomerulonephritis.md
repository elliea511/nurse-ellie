---
layout: default
title: Glomerulonephritis
---

<div class="mh-breadcrumb">
  <a href="{{ '/renal-urinary.html' | relative_url }}">Renal & Urinary</a>
  <span>Glomerulonephritis</span>
</div>

# Glomerulonephritis

<p class="mh-subtitle">PN-level review of post-streptococcal glomerulonephritis, hematuria, proteinuria, edema, hypertension, fluid balance, treatment, and priority nursing care</p>

**Glomerulonephritis** is inflammation and damage of the glomeruli, the tiny filtering structures inside the kidneys. When the glomeruli become inflamed, blood and protein can leak into the urine while sodium, water, and waste products remain in the bloodstream.

For the PN unit exam, the major focus is **acute post-streptococcal glomerulonephritis**, which can develop after a recent group A streptococcal throat or skin infection.

<div class="priority-focus">
  <div>
    <h3>Priority Focus</h3>
    Recent Strep Infection • Cola-Colored Urine • Edema • Hypertension • Oliguria • Daily Weight • Intake & Output • Kidney Function
  </div>
</div>

<div class="mh-cards">

  <div class="mh-card blue">
    <div class="mh-card-icon">🧬</div>
    <h3>Cause & Pathophysiology</h3>
    <p>An immune response after a streptococcal infection damages the kidney’s glomeruli.</p>
    <a class="mh-btn" href="#panel-cause">Study the Cause</a>
  </div>

  <div class="mh-card green">
    <div class="mh-card-icon">🩸</div>
    <h3>Assessment Findings</h3>
    <p>Hematuria, proteinuria, oliguria, edema, hypertension, fatigue, and fluid retention.</p>
    <a class="mh-btn" href="#panel-assessment">Study Assessment</a>
  </div>

  <div class="mh-card pink">
    <div class="mh-card-icon">⚖️</div>
    <h3>Treatment & Nursing Care</h3>
    <p>Fluid balance, blood-pressure control, dietary restrictions, medications, and monitoring.</p>
    <a class="mh-btn" href="#panel-treatment">Study Nursing Care</a>
  </div>

</div>

<div class="mh-detail blue" id="panel-cause" markdown="1">
<a class="mh-detail-close" href="#">✕</a>

### Cause and Pathophysiology

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### What It Is

* Inflammation of the glomeruli.
* The glomeruli normally filter the blood.
* Inflammation makes the filters more permeable.
* Blood and protein leak into the urine.
* Kidney filtration decreases.
* Sodium, water, and wastes remain in the body.

</div>

<div markdown="1">
##### Post-Streptococcal Cause

May occur after:

* Streptococcal pharyngitis
* Strep throat
* Scarlet fever
* Streptococcal skin infection
* Impetigo

It commonly develops after the original infection has already begun improving.

</div>

<div markdown="1">
##### Immune Response

* The body produces antibodies against group A streptococcus.
* Antigen-antibody complexes become trapped in the glomeruli.
* The immune response causes inflammation.
* The kidney itself is not directly infected with strep bacteria.
* This is different from pyelonephritis.

</div>

<div markdown="1">
##### Who Is Commonly Affected?

* More common in children.
* Often follows a recent throat or skin infection.
* Adults can also develop it.
* Adults may have a greater risk of lasting kidney impairment.

</div>

</div>

<div class="detail-table">
  <table>
    <thead>
      <tr>
        <th>Pyelonephritis</th>
        <th>Glomerulonephritis</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>Kidney infection</td>
        <td>Inflammation of the kidney filters</td>
      </tr>
      <tr>
        <td>Usually caused by bacteria ascending from the bladder</td>
        <td>Often caused by an immune reaction after strep</td>
      </tr>
      <tr>
        <td>Fever, chills, flank pain, and CVA tenderness</td>
        <td>Edema, hypertension, hematuria, and proteinuria</td>
      </tr>
      <tr>
        <td>Urinary frequency, urgency, and dysuria may occur</td>
        <td>Urine output may decrease</td>
      </tr>
      <tr>
        <td>Antibiotics treat the kidney infection</td>
        <td>Treatment mainly controls fluid retention and blood pressure</td>
      </tr>
    </tbody>
  </table>
</div>

#### Timing After Infection

Post-streptococcal glomerulonephritis may develop:

* Approximately 1 to 2 weeks after strep throat
* Up to several weeks after a streptococcal skin infection
* After the client thinks the original infection has resolved

Ask about:

* Recent sore throat
* Recent fever
* Recent impetigo
* Recent skin infection
* Recent antibiotic treatment
* Family or school exposure to strep

<div class="detail-callout" markdown="1">
<strong>Class Cue:</strong> A child who recently had strep throat and now has puffy eyes, hypertension, decreased urine output, and dark urine should be evaluated for acute post-streptococcal glomerulonephritis.
</div>

</div>

<div class="mh-detail green" id="panel-assessment" markdown="1">
<a class="mh-detail-close" href="#">✕</a>

### Assessment Findings

The major findings result from:

1. Blood and protein leaking through the damaged glomeruli
2. Decreased kidney filtration
3. Sodium and water retention

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Urinary Findings

* Hematuria
* Dark, smoky, tea-colored, or cola-colored urine
* Proteinuria
* Oliguria
* Red blood cell casts
* Increased urine concentration in some clients

</div>

<div markdown="1">
##### Fluid Retention

* Periorbital edema
* Facial puffiness
* Hand and foot swelling
* Dependent edema
* Rapid weight gain
* Hypertension
* Possible lung crackles

</div>

<div markdown="1">
##### General Findings

* Fatigue
* Lethargy
* Weakness
* Decreased appetite
* Headache
* Pallor
* Mild fever may occur
* Generalized discomfort

</div>

<div markdown="1">
##### Priority Dangers

* Severe hypertension
* Pulmonary edema
* Heart failure
* Hyperkalemia
* Acute kidney injury
* Hypertensive encephalopathy
* Seizures
* Fluid overload

</div>

</div>

<div class="detail-table">
  <table>
    <thead>
      <tr>
        <th>Finding</th>
        <th>Why It Occurs</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><strong>Hematuria</strong></td>
        <td>Inflamed glomeruli allow red blood cells to leak into the urine.</td>
      </tr>
      <tr>
        <td><strong>Cola-colored urine</strong></td>
        <td>Blood inside the urine creates a dark or smoky appearance.</td>
      </tr>
      <tr>
        <td><strong>Proteinuria</strong></td>
        <td>Protein leaks through the damaged filtration membrane.</td>
      </tr>
      <tr>
        <td><strong>Oliguria</strong></td>
        <td>Glomerular filtration decreases, producing less urine.</td>
      </tr>
      <tr>
        <td><strong>Edema</strong></td>
        <td>Sodium and water remain in the body.</td>
      </tr>
      <tr>
        <td><strong>Periorbital edema</strong></td>
        <td>Fluid commonly collects around the eyes, especially in the morning.</td>
      </tr>
      <tr>
        <td><strong>Hypertension</strong></td>
        <td>Excess circulating fluid raises blood pressure.</td>
      </tr>
      <tr>
        <td><strong>Weight gain</strong></td>
        <td>Rapid gain usually reflects retained fluid rather than fat.</td>
      </tr>
      <tr>
        <td><strong>Elevated BUN and creatinine</strong></td>
        <td>The kidneys are not filtering metabolic waste effectively.</td>
      </tr>
    </tbody>
  </table>
</div>

#### Expected Laboratory and Diagnostic Findings

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Urinalysis

May show:

* Blood
* Protein
* Red blood cells
* Red blood cell casts
* Increased concentration
* Reduced urine volume

</div>

<div markdown="1">
##### Kidney Function

May show:

* Elevated BUN
* Elevated serum creatinine
* Decreased estimated filtration
* Electrolyte changes
* Possible hyperkalemia

</div>

<div markdown="1">
##### Evidence of Recent Strep

Testing may include:

* Throat or skin culture
* Antistreptolysin O titer
* Other streptococcal antibody testing
* History of recent infection

</div>

<div markdown="1">
##### Other Possible Findings

* Decreased complement C3
* Mild anemia
* Decreased serum protein
* Elevated inflammatory markers
* Chest x-ray if pulmonary edema is suspected

</div>

</div>

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Acute Edematous Phase

This is the early phase in which kidney filtration is reduced.

Expected findings include:

* Oliguria
* Hematuria
* Proteinuria
* Edema
* Rapid weight gain
* Hypertension
* Elevated BUN and creatinine
* Possible hyperkalemia
* Possible fluid overload

</div>

<div markdown="1">
##### Recovery or Diuretic Phase

As kidney filtration improves:

* Urine output increases.
* Excess fluid begins leaving the body.
* Edema decreases.
* Weight decreases.
* Blood pressure may improve.
* Kidney-function labs gradually improve.

The client still requires monitoring because excessive diuresis can cause:

* Dehydration
* Hypotension
* Electrolyte imbalance
* Weakness
* Falls

</div>

</div>

<div class="detail-callout" markdown="1">
<strong>Phase Comparison:</strong> The acute phase produces oliguria and edema. Improvement is marked by increased urine output and decreasing edema.
</div>

#### Focused Nursing Assessment

Assess:

* Blood pressure
* Heart rate
* Respiratory rate
* Oxygen saturation
* Lung sounds
* Edema location and severity
* Daily weight
* Intake and output
* Urine color and amount
* Neurologic status
* Headache
* Vision changes
* Seizure activity
* Potassium
* BUN and creatinine

<div class="report-now" markdown="1">
<h3>Report Immediately</h3>

* Severe hypertension
* Severe headache
* Blurred vision
* New confusion
* Seizure
* Shortness of breath
* Crackles
* Falling oxygen saturation
* Chest pain
* Rapidly increasing edema
* Very low or absent urine output
* Hyperkalemia or dysrhythmia

</div>

<div class="detail-callout" markdown="1">
<strong>NCLEX Priority:</strong> New crackles or shortness of breath in a client with glomerulonephritis can indicate pulmonary edema and takes priority over urine discoloration.
</div>

</div>

<div class="mh-detail pink" id="panel-treatment" markdown="1">
<a class="mh-detail-close" href="#">✕</a>

### Treatment and Nursing Care

Treatment focuses on:

* Controlling fluid retention
* Managing hypertension
* Monitoring kidney function
* Preventing complications
* Treating any remaining streptococcal infection
* Supporting the client until kidney function improves

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Intake and Output

* Measure all oral and IV intake.
* Measure urine output accurately.
* Note oliguria or anuria.
* Observe urine color.
* Report sudden changes.
* Follow prescribed fluid restriction.

</div>

<div markdown="1">
##### Daily Weight

* Weigh at the same time every day.
* Use the same scale.
* Use similar clothing.
* Weigh after voiding when possible.
* Record trends.
* Rapid gain suggests fluid retention.

</div>

<div markdown="1">
##### Vital Signs

* Monitor blood pressure frequently.
* Assess for severe headache or visual changes.
* Monitor heart rate and rhythm.
* Assess respiratory status.
* Report worsening hypertension.

</div>

<div markdown="1">
##### Edema and Respiratory Status

* Assess periorbital and dependent edema.
* Check lung sounds.
* Monitor oxygen saturation.
* Elevate swollen extremities when appropriate.
* Reposition regularly.
* Protect edematous skin.

</div>

</div>

#### Diet and Fluid Management

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Low-Sodium Diet

Sodium may be limited to reduce:

* Fluid retention
* Edema
* Hypertension
* Cardiac workload

Avoid heavily salted and processed foods.

</div>

<div markdown="1">
##### Fluid Restriction

May be ordered when the client has:

* Oliguria
* Significant edema
* Hypertension
* Pulmonary congestion
* Rapid weight gain

Include liquids that become fluid at room temperature.

</div>

<div markdown="1">
##### Potassium Restriction

May be needed when:

* Urine output is low
* Serum potassium is elevated
* Kidney filtration is reduced
* The client has ECG changes

Do not restrict potassium without an order or laboratory indication.

</div>

<div markdown="1">
##### Protein

* Protein needs depend on kidney function and losses.
* A moderate restriction may be ordered if nitrogen waste rises.
* Adequate nutrition is still required.
* Follow the prescribed renal diet.
* Avoid giving a blanket high- or low-protein instruction.

</div>

</div>

#### Medications

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Diuretics

Example: **Furosemide**

Purpose:

* Increase urine output
* Reduce edema
* Reduce blood pressure
* Decrease pulmonary congestion

Monitor:

* Blood pressure
* Intake and output
* Daily weight
* Potassium
* Dehydration

</div>

<div markdown="1">
##### Antihypertensives

May be prescribed to control blood pressure.

Monitor:

* Blood pressure before administration
* Dizziness
* Orthostatic hypotension
* Kidney function
* Potassium with certain medications

</div>

<div markdown="1">
##### Antibiotics

* Antibiotics do not reverse glomerular inflammation.
* They may be prescribed if group A strep is still present.
* Treatment helps eliminate remaining bacteria.
* Complete the full course.
* Follow infection-control teaching.

</div>

<div markdown="1">
##### Emergency Medications

Severe complications may require treatment for:

* Hypertensive emergency
* Hyperkalemia
* Pulmonary edema
* Seizure
* Acute kidney injury

Dialysis is rarely needed but may be required if severe kidney failure develops.

</div>

</div>

<div class="renal-two-column" markdown="1">

<div markdown="1">
##### Nursing Interventions

* Maintain accurate intake and output.
* Obtain daily weights.
* Monitor blood pressure frequently.
* Monitor urine appearance and output.
* Assess edema.
* Auscultate lung sounds.
* Monitor oxygen saturation.
* Review BUN, creatinine, potassium, and urinalysis results.
* Administer medications as ordered.
* Implement sodium, fluid, or potassium restrictions as prescribed.
* Provide rest during the acute phase.
* Cluster care to reduce fatigue.
* Protect edematous skin.
* Use seizure precautions if severe hypertension or neurologic symptoms occur.
* Teach the client and family what findings require immediate attention.

</div>

<div markdown="1">
##### Activity and Rest

During the acute phase:

* Rest may be needed.
* Activity may be limited with severe edema or hypertension.
* Cluster nursing care.
* Allow quiet periods.
* Increase activity gradually as edema and blood pressure improve.
* Use fall precautions if weakness or diuresis causes dizziness.

</div>

<div markdown="1">
##### Patient and Family Education

* Monitor urine amount and color.
* Monitor blood pressure if instructed.
* Record daily weights when prescribed.
* Follow sodium and fluid restrictions.
* Take all medications exactly as directed.
* Complete prescribed antibiotics.
* Keep follow-up laboratory appointments.
* Report reduced urine output.
* Report sudden weight gain or worsening swelling.
* Report shortness of breath.
* Report severe headache, confusion, or vision changes.
* Seek treatment for future symptoms of strep throat or skin infection.

</div>

</div>

<div class="detail-callout" markdown="1">
<strong>Teaching Cue:</strong> Daily weight is the best simple measure of overall fluid gain or loss. Intake and output help explain where the fluid is going.
</div>

#### Prognosis

* Most children recover completely.
* Urine findings may take time to return fully to normal.
* Blood pressure and kidney function require follow-up.
* Some adults have a greater risk of lasting kidney damage.
* A small number of clients may develop chronic kidney disease or kidney failure.

<div class="detail-callout" markdown="1">
<strong>NCLEX:</strong> Improvement is shown by increased urine output, reduced edema, decreasing weight, improving blood pressure, and improving BUN and creatinine—not simply by lighter urine color.
</div>

</div>

<div class="mh-columns wide-left" markdown="1">

<div class="key-findings" markdown="1">
<h3>📋 Glomerulonephritis Big Picture</h3>

* The glomeruli are the kidney’s filtering structures.
* Post-streptococcal glomerulonephritis is an immune response after strep throat or a skin infection.
* It is not an ascending bacterial UTI.
* Inflamed glomeruli allow blood and protein to enter the urine.
* Decreased filtration causes oliguria, edema, hypertension, and elevated waste levels.
* Cola-colored urine is a classic finding.
* Periorbital edema is often noticeable in the morning.
* Monitor blood pressure, daily weight, intake and output, edema, lung sounds, and kidney labs.
* The acute phase causes oliguria and fluid retention.
* The recovery phase causes increased urine output and decreased edema.

</div>

<div class="mh-col" markdown="1">

<div class="report-now" markdown="1">
<h3>Report Immediately</h3>

* Severe or rapidly rising blood pressure
* Severe headache or blurred vision
* New confusion or seizure
* Shortness of breath
* Crackles or falling oxygen saturation
* Chest pain
* Very low or absent urine output
* Rapid weight gain
* Worsening generalized edema
* Hyperkalemia or dysrhythmia

</div>

<div class="nclex-tip" markdown="1">
<h3>NCLEX Tip</h3>

Do not confuse the two kidney conditions:

**Pyelonephritis:** Infection, fever, flank pain, CVA tenderness.

**Glomerulonephritis:** Immune inflammation, hematuria, proteinuria, edema, hypertension, and oliguria.

</div>

</div>
</div>

<div class="mh-table-card" markdown="1">

<h3>Quick Terms</h3>

| Term                      | Simple Meaning                                                 |
| ------------------------- | -------------------------------------------------------------- |
| **Glomerulus**            | Tiny structure that filters blood inside the kidney            |
| **Glomerulonephritis**    | Inflammation of the glomeruli                                  |
| **Post-streptococcal**    | Occurring after a group A strep infection                      |
| **Immune complex**        | Antigen and antibody material that can trigger inflammation    |
| **Hematuria**             | Blood in the urine                                             |
| **Proteinuria**           | Protein in the urine                                           |
| **Oliguria**              | Abnormally low urine output                                    |
| **Anuria**                | Little to no urine production                                  |
| **Periorbital edema**     | Swelling around the eyes                                       |
| **RBC cast**              | Mold of red blood cells formed in kidney tubules               |
| **Azotemia**              | Increased nitrogen wastes in the blood                         |
| **Acute edematous phase** | Oliguric phase with edema and fluid retention                  |
| **Diuretic phase**        | Recovery period with increased urine output                    |
| **Complement C3**         | Immune-system protein that may be decreased in PSGN            |
| **ASO titer**             | Blood test that can support evidence of recent strep infection |

</div>

<div class="mh-columns" markdown="1">

<div class="quick-review" markdown="1">
<h3>Quick Review</h3>

1. Glomerulonephritis affects the kidney filters.
2. Acute post-streptococcal disease follows a recent throat or skin infection.
3. It is caused by an immune response, not bacteria climbing from the bladder.
4. Classic urine is smoky, tea-colored, or cola-colored.
5. Expect hematuria, proteinuria, oliguria, edema, and hypertension.
6. Monitor daily weight, intake and output, blood pressure, lung sounds, and kidney labs.
7. Sodium and fluid may be restricted.
8. Potassium may be restricted when oliguria or hyperkalemia is present.
9. Diuretics and antihypertensives may be prescribed.
10. Increased urine output with decreasing edema indicates recovery.

</div>

<div class="do-not-miss" markdown="1">
<h3>Do Not Miss</h3>

* Do not confuse glomerulonephritis with pyelonephritis.
* Do not ignore rapidly increasing blood pressure.
* Do not overlook crackles or shortness of breath.
* Do not assume dark urine is the only important finding.
* Do not push unrestricted fluids when the client is oliguric and edematous.
* Do not give high-potassium foods when hyperkalemia is present.
* Do not skip daily weights.
* Do not assume antibiotics alone treat the kidney inflammation.
* Do not ignore neurologic changes; severe hypertension can cause seizures.

</div>

</div>

<div class="cheer-banner">
  🩵 Glomerulonephritis questions follow one pattern: recent strep → damaged filters → blood and protein in urine → fluid stays in the body → edema and hypertension. 🩵
</div>

---

<div class="page-nav">
  <a href="{{ '/renal-urinary/kidney-stones.html' | relative_url }}" class="page-nav-back">← Kidney Stones</a>
  <a href="{{ '/renal-urinary/acute-chronic-kidney-disease.html' | relative_url }}" class="page-nav-next">Acute & Chronic Kidney Disease →</a>
</div>
