---
layout: default
title: Home
type: index
---

<div class="girly-blob-pink"></div>
<div class="girly-blob-mauve"></div>
<div class="girly-blob-blue"></div>

<div class="sng-main-wrapper">

  <!-- LEFT SIDE: TITLE & BUTTONS -->
  <div class="sng-hero-content">
    <div class="sng-hero-title">Nurse Ellie’s<br>Study Lounge</div>
    <div class="sng-hero-desc">
      Simple nursing notes and review tools made for exam prep.<br>
      <span style="color:#8bb488;font-weight:900;background:#f8fff5;padding:1px 11px;border-radius:9px;">Organized so you can study what you actually need.</span>
    </div>
    <div class="sng-hero-actions">
      <a href="/maternity/" class="sng-call-btn">Start Studying</a>
      <a href="/practice/" class="sng-call-btn secondary">Quick Review</a>
    </div>
  </div>

  <!-- RIGHT SIDE: TILES -->
  <div class="sng-card-grid">
    <a href="/maternity/" class="sng-card">
      <div class="sng-card-title">Maternity</div>
      <div class="sng-card-desc">Antepartum, intrapartum, postpartum, newborn, meds, labs, and practice questions.</div>
    </a>
    <a href="/hematology-oncology/" class="sng-card">
      <div class="sng-card-title">Hematology / Oncology</div>
      <div class="sng-card-desc">Anemias, cancer care, emergencies, and high-yield review.</div>
    </a>
    <a href="/medical-emergencies/" class="sng-card">
      <div class="sng-card-title">Medical Emergencies</div>
      <div class="sng-card-desc">Burns, shock, increased ICP, hydrocephalus, interventions, and priorities.</div>
    </a>
    <a href="/ati/" class="sng-card">
      <div class="sng-card-title">ATI Study Guides</div>
      <div class="sng-card-desc">Nutrition, exam review notes, and NCLEX-style study guides.</div>
    </a>
  </div>

</div>
